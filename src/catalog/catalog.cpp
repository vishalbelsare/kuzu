...
template<>
uint64_t SerDeser::serializeValue<std::string>(const std::string& value, FileInfo* fileInfo,
    uint64_t offset) {
    uint64_t valueLength = value.length();
    FileUtils::writeToFile(fileInfo, (uint8_t*)&valueLength, sizeof(uint64_t), offset);
    FileUtils::writeToFile(fileInfo, (uint8_t*)value.data(), valueLength,
        offset + sizeof(uint64_t));
    return offset + sizeof(uint64_t) + valueLength;
}

template<>
uint64_t SerDeser::deserializeValue<std::string>(std::string& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::serializeValue<Property>(const Property& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::deserializeValue<Property>(Property& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::serializeValue(const std::unordered_map<table_id_t, uint64_t>& value,
...
template<>
uint64_t SerDeser::deserializeValue(std::unordered_map<table_id_t, uint64_t>& value,
    FileInfo* fileInfo, uint64_t offset) {
...
template<>
uint64_t SerDeser::serializeValue<TableSchema>(const TableSchema& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::deserializeValue<TableSchema>(TableSchema& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::serializeValue<NodeTableSchema>(const NodeTableSchema& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::deserializeValue<NodeTableSchema>(NodeTableSchema& value, FileInfo* fileInfo,
    uint64_t offset) {
...
template<>
uint64_t SerDeser::serializeValue<RelTableSchema>(const RelTableSchema& value, FileInfo* fileInfo,
    uint64_t offset) {
    offset = SerDeser::serializeValue<TableSchema>((const TableSchema&)value, fileInfo, offset);
...
template<>
uint64_t SerDeser::deserializeValue<RelTableSchema>(RelTableSchema& value, FileInfo* fileInfo,
    uint64_t offset) {
    offset = SerDeser::deserializeValue<TableSchema>((TableSchema&)value, fileInfo, offset);
...
CatalogContent::CatalogContent(const std::string& directory) {
    ...
    auto catalogPath = StorageUtils::getCatalogFilePath(directory, DBFileType::ORIGINAL);
    if (FileUtils::fileOrPathExists(catalogPath)) {
        readFromFile(catalogPath);
    } else {
        saveToFile(catalogPath);
    }
...
table_id_t CatalogContent::addNodeTableSchema(std::string tableName, property_id_t primaryKeyId,
    ...
    auto nodeTableSchema = std::make_unique<NodeTableSchema>(std::move(tableName), tableID,
        primaryKeyId, std::move(properties));
...
const Property& CatalogContent::getNodeProperty(table_id_t tableID,
...
const Property& CatalogContent::getRelProperty(table_id_t tableID,
    const std::string& propertyName) const {
...
void CatalogContent::saveToFile(const std::string& path) {
    auto fileInfo = FileUtils::openFile(path, O_WRONLY | O_CREAT);
    uint64_t offset = 0;
    writeMagicBytes(fileInfo.get(), offset);
    offset = SerDeser::serializeValue<uint64_t>(StorageVersionInfo::getStorageVersion(),
        fileInfo.get(), offset);
    ...
    for (auto& nodeTableSchema : nodeTableSchemas) {
        ...
        offset = SerDeser::serializeValue<NodeTableSchema>(*nodeTableSchema.second, fileInfo.get(),
            offset);
    }
    for (auto& relTableSchema : relTableSchemas) {
        offset = SerDeser::serializeValue<uint64_t>(relTableSchema.first, fileInfo.get(), offset);
        offset = SerDeser::serializeValue<RelTableSchema>(*relTableSchema.second, fileInfo.get(),
            offset);
    }
    SerDeser::serializeValue<table_id_t>(nextTableID, fileInfo.get(), offset);
}

void CatalogContent::readFromFile(const std::string& path) {
    logger->debug("Reading from {}.", path);
    auto fileInfo = FileUtils::openFile(path, O_RDONLY);
    uint64_t offset = 0;
    ...
    for (auto i = 0u; i < numNodeTables; i++) {
        ...
        offset = SerDeser::deserializeValue<NodeTableSchema>(*nodeTableSchemas[tableID],
            fileInfo.get(), offset);
    }
    for (auto i = 0u; i < numRelTables; i++) {
        offset = SerDeser::deserializeValue<uint64_t>(tableID, fileInfo.get(), offset);
        relTableSchemas[tableID] = std::make_unique<RelTableSchema>();
        offset = SerDeser::deserializeValue<RelTableSchema>(*relTableSchemas[tableID],
            fileInfo.get(), offset);
    }
...
table_id_t Catalog::addNodeTableSchema(std::string tableName, property_id_t primaryKeyId,
    ...
    auto tableID = catalogContentForWriteTrx->addNodeTableSchema(std::move(tableName), primaryKeyId,
...
table_id_t Catalog::addRelTableSchema(std::string tableName, RelMultiplicity relMultiplicity,
    ...
    auto tableID = catalogContentForWriteTrx->addRelTableSchema(std::move(tableName),
        relMultiplicity, propertyDefinitions, srcTableID, dstTableID);
...
void Catalog::addProperty(table_id_t tableID, std::string propertyName, DataType dataType) {
    initCatalogContentForWriteTrxIfNecessary();
    catalogContentForWriteTrx->getTableSchema(tableID)->addProperty(propertyName,
        std::move(dataType));
...
void Catalog::renameProperty(table_id_t tableID, property_id_t propertyID,
